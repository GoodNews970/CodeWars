...
or gas lost every day (evap_per_day) and the threshold (threshold) in percentage
 beyond which the evaporator is no longer useful. All numbers are strictly
 positive.
...
prefer to reason with content, some other with percentages only. It's up to you
...
public:
    static int evaporator(double content, double evap_per_day, double threshold)
    {
        int days = 0;
        double baseline = 100;
        while (baseline >= threshold)
        {
            baseline *= (1-evap_per_day/100);
            days++;
        }
        return days;
    }
